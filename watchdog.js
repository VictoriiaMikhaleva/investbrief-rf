 ...
 * Облако (опционально): Firestore meta/watchdog — нужен Firebase Blaze.
 */
(function () {
  'use strict';

  var WATCHDOG_KEY = 'ibrf.watchdog';
  /** Облачный сторож отключён, пока нет деплоя на Blaze */
  var CLOUD_WATCHDOG_ENABLED = false;
  ...
  function onVisibilityWake() {
    ...
    if (CLOUD_WATCHDOG_ENABLED) checkCloudWatchdogSignal('visibility-cloud');
  }
  ...
  function syncWatchdogCloudUi() {
    var showCloud = CLOUD_WATCHDOG_ENABLED;
    var cloudBtn = document.getElementById('watchdogCloudCheckBtn');
    var cloudHint = document.getElementById('watchdogCloudHint');
    if (cloudBtn) cloudBtn.hidden = !showCloud;
    if (cloudHint) cloudHint.hidden = !showCloud;
  }
  ...
  function updateWatchdogDevUI(cfg, cloudMeta) {
    ...
    if (!CLOUD_WATCHDOG_ENABLED) {
      statusEl.textContent = localLine;
      return;
    }
    ...
    var cloudLine = cloudMs
      ...
      : 'Облако: нет метки. ';
    statusEl.textContent = cloudLine + localLine;
  }
  ...
  function bindWatchdogDevUI() {
    ...
    if (cloudBtn && CLOUD_WATCHDOG_ENABLED) {
  ...
  function bindFirebaseWatchdog() {
    if (!CLOUD_WATCHDOG_ENABLED) return;
